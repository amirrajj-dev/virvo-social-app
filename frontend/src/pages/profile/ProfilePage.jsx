  ...
  return (
    <div className="p-4 lg:p-8">
      ...
      <div className="border-b border-gray-300 dark:border-gray-700 mt-4">
        <button onClick={() => handleTabChange('posts')} className={`tab ${activeTab === 'posts' ? 'tab-active' : ''}`}>
          Posts
        </button>
        <button onClick={() => handleTabChange('likes')} className={`tab ${activeTab === 'likes' ? 'tab-active' : ''}`}>
          Likes
        </button>
      </div>
      <div className="mt-4 overflow-auto max-h-[470px]">
        {activeTab === 'posts' && (
          userPosts?.length > 0 
            ? userPosts.map(post => (
                <Post key={post.id} {...post} isMyPost={true} deletePost={() => deletePost(post._id)} onLike={() => {}} onSave={() => {}} />
              ))
            : (isLoadingPosts ? <PostSkeleton /> : <div>No posts available</div>)
        )}
        {activeTab === 'likes' && (
          likedPosts?.length > 0 
            ? likedPosts.map(post => (
                <Post key={post.id} post={post} user={user} isMyPost={false} onDelete={() => {}} onLike={() => {}} onSave={() => {}} />
              ))
            : <PostSkeleton />
        )}
      </div>
